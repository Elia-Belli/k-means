...
for ((i=0; i < TEST_RUN; i++));
  do
    ...
    if [ $RUN_SEQUENTIAL_TESTS == true ]; then
      echo "[${i}] Running Sequential version"
      for ((j=0; j < INPUT_NUM; j++));
      do
        echo "[SEQUENTIAL] Running test ${j}"
          {\
            ./bin/KMEANS_seq "${INPUT[j]}" "${K[j]}" "$ITER" "$MIN_CHANGES" "$MAX_DIST" "${OUT_DIR}KMEANS_seq_${j}.txt"; \
            printf "\n"; \
          } >> "${TEST_RESULTS}input_${j}.txt"
      done
      echo "[${i}] Sequential runs completed"
    fi
    ...
    if [ $RUN_MPI_TESTS == true ]; then
      ...
      do
        ...
        {\
          mpirun --np "${MPI_PROCESSES}" --oversubscribe ./bin/KMEANS_mpi "${INPUT[j]}" "${K[j]}" "$ITER" "$MIN_CHANGES" "$MAX_DIST" "${OUT_DIR}KMEANS_mpi_${j}.txt" 2>"${OUT_DIR}KMEANS_mpi_${j}_error.txt"; \
          printf ","; \
